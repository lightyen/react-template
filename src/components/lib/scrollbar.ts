...
export function setScroll(hide: boolean, needFix: boolean = !isMobile() && !isFirefox()) {
	...
	if (needFix) {
		...
		if (toaster) {
			if (hide || !canScroll) {
				toaster.style.marginRight = `${sw}px`
			} else {
				toaster.style.marginRight = ""
			}
		}
	}
}
...
function setToastMargin(needFix: boolean = !isMobile() && !isFirefox()) {
	if (!needFix) {
		return
	}
	const el = document.getElementById("toaster")
	if (el) {
		if (!scrollable(document.documentElement) || getDialogCount() > 0) {
			el.style.marginRight = `${getScrollbarWidth(getViewportElement())}px`
		} else {
...
const observer = new ResizeObserver(() => {
	// no scroll -> marginRight toaster
	// scroll -> no marginRight toaster
	setToastMargin()
})

setToastMargin()
observer.observe(getViewportElement())
